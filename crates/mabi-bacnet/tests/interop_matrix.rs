...
#[test]
#[ignore]
fn repo_local_bacnet_interop_matrix_executes_available_targets() {
    ...
    if let Err(reason) = &docker_ready {
        if std::env::var_os("CI").is_some() {
            panic!("self-contained bacnet interop requires docker in CI: {reason}");
        }
        ...
        eprintln!(
            "bacnet self-contained interop matrix: passed=0, failed=0, skipped={}",
            skipped.len()
        );
    ...
    eprintln!(
        "bacnet self-contained interop matrix: passed={}, failed={}, skipped={}",
        passed.len(),
        failed.len(),
        skipped.len()
    );
    ...
    assert!(
        failed.is_empty(),
        "bacnet self-contained interop matrix failures: {}",
        failed.join(", ")
    );
}
